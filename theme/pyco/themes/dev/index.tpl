...
<div id="wrapper" class="main-bg">
  ...
  <article class="container text-center summary">
    <h1 sup-editor-meta ng-model="meta.title"></h1>
    <div sup-angular-wysiwyg ng-model="content"></div>
    <aside>
      <a href="#" ng-repeat="item in menu['primary']"
      ng-if="(item.url|path) != (base_url|path)"
      class="btn-nav btn-nav-lg {{item.meta.class}}">{{item.title}}</a>
    </aside>
  </article>
  <!-- Register -->
